...
struct Entry<K, V> {
	ptr: AtomicCell<Arc<Option<RawEntry<K, V>>>>,
}

impl<K, V> Entry<K, V> {
	fn empty() -> Self {
		Self {
			ptr: AtomicCell::new(Arc::new(None)),
		}
	}
	...
	fn swap(&self, raw: Option<RawEntry<K, V>>) -> Option<RawEntry<K, V>> {
		if let Ok(entry) = Arc::try_unwrap(self.ptr.swap(Arc::new(raw))) {
			entry
		} else {
			None
		}
	...
	fn load(&self) -> &Option<RawEntry<K,V>> {
	...
	K: std::cmp::PartialEq + std::hash::Hash + std::clone::Clone + std::fmt::Display,
{
	...
	pub fn insert(&self, key: K, value: V) -> Result<(), ()> {
		...
		for hash_id in 0..4 {
			...
			if let Some(existing) = self.data[position].load() {
				if existing.key == key {
					...
					if let Some(previous) = self.data[position].swap(Some(new)) {
						if previous.key != key {
		...
		for position in &positions {
			...
			if let Some(current_key) = self.data[*position].load().as_ref().map(|v| v.key.clone()) {
				...
				for hash_id in 0..4 {
					...
					if self.data[next_position].is_empty() {
						let new = RawEntry::new(key.clone(), value);
						if let Some(current) = self.data[*position].swap(Some(new)) {
							// but we may have raced, so check
							if let Some(next) = self.data[next_position].swap(Some(RawEntry::new(current.key.clone(), current.value))) {
...
where
	K: std::cmp::PartialEq + std::hash::Hash + std::clone::Clone + std::fmt::Display,
{
    fn new(inner: &'a Table<K, V>) -> Iter<'a, K, V> {
        Iter { inner, index: 0 }
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
    	loop {
    		if self.index < self.inner.data.len() {
    			let ret = self.inner.data[self.index].load();
    			self.index += 1;
...
mod tests {
    ...
    #[test]
    fn basic() {
        let map = Table::<u64, u64>::with_capacity(128);
    ...
    fn fill() {
    	...
    	let map = Table::<u8, u64>::with_capacity(u8::MAX as usize * 2);
    	for k in 0..u8::MAX {
    		let _ = map.insert(k, 0);
    	}

    	// table needs more room than you'd think
    	let map = Table::<u16, u64>::with_capacity(u16::MAX as usize * 2);
    	for k in 0..u16::MAX {
    ...
    fn occupancy() {
    	...
    	let map = Table::<u32, u64>::with_capacity(capacity as usize);
    	let mut inserted = 0;
    	for k in 0..capacity {
    ...
    fn coherence() {
    	let capacity: u32 = 1_000_000;
    	let map = Arc::new(Table::<u32, u32>::with_capacity(capacity as usize));

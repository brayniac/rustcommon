                    ...
                    if self.data[next_position].is_empty() {
                        ...
                        if let Some(current) = self.data[*position].swap(Some(new)) {
                            ...
                            if let Some(next) = self.data[next_position]
                                .swap(Some(RawEntry::new(current.key.clone(), current.value)))
                            {
...
where
    K: std::cmp::PartialEq + std::hash::Hash + std::clone::Clone + std::fmt::Display,
{
    fn new(inner: &'a HashMap<K, V>) -> Iter<'a, K, V> {
        Iter { inner, index: 0 }
    }
}
...
mod tests {
    ...
    #[test]
    fn basic() {
        let map = HashMap::<u64, u64>::with_capacity(128);
    ...
    fn fill() {
        ...
        let map = HashMap::<u8, u64>::with_capacity(u8::MAX as usize * 2);
        for k in 0..u8::MAX {
            let _ = map.insert(k, 0);
        }

        // table needs more room than you'd think
        let map = HashMap::<u16, u64>::with_capacity(u16::MAX as usize * 2);
        for k in 0..u16::MAX {
    ...
    fn occupancy() {
        ...
        let map = HashMap::<u32, u64>::with_capacity(capacity as usize);
        let mut inserted = 0;
        for k in 0..capacity {
    ...
    fn coherence() {
        let capacity: u32 = 1_000_000;
        let map = Arc::new(HashMap::<u32, u32>::with_capacity(capacity as usize));
